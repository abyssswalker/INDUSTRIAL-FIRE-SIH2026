...
def get_osm_id(feature: dict) -> str | None:
    ...
    for key in ("id", "@id", "osm_id", "osm3s_id"):
...
def get_feature_name(properties: dict) -> str | None:
    for key in ("name", "official_name", "alt_name", "operator"):
        value = properties.get(key)
        if value:
            return str(value)
    return None
...
def get_feature_type(properties: dict) -> str:
    preferred_tags = ["landuse", "industrial", "power", "man_made", "building", "amenity"]
...
def get_point_coordinates(feature: dict):
    ...
    if geometry_type == "MultiPolygon":
        ...
        longitude = sum(point[0] for point in first_polygon) / len(first_polygon)
        latitude = sum(point[1] for point in first_polygon) / len(first_polygon)
        return longitude, latitude
...
def import_osm_geojson(geojson_path: str) -> int:
    ...
    if not path.exists():
        raise FileNotFoundError(f"OSM GeoJSON was not found: {path}")
        ...
        for feature in features:
            ...
            statement = text(
                ...
                INSERT INTO osm_features (
                    osm_id, name, feature_type, latitude, longitude,
                    properties, geom, source, updated_at
                )
                VALUES (
                    :osm_id, :name, :feature_type, :latitude, :longitude,
...
if __name__ == "__main__":
    geojson_file = sys.argv[1] if len(sys.argv) > 1 else "data/osm_features.geojson"
    count = import_osm_geojson(geojson_file)
    print(f"Imported or updated {count} OSM features.")